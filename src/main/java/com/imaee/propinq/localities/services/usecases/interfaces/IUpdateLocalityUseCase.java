package com.imaee.propinq.localities.services.usecases.interfaces;

import com.imaee.propinq.localities.controllers.requests.LocalityRequest;
import com.imaee.propinq.localities.data.models.Locality;

import java.util.UUID;

public interface IUpdateLocalityUseCase {
    void updateLocality(LocalityRequest LocalityRequest, UUID localityId);
    void throwExceptionIfLocalityIsDeleted(Locality locality);
}
